...
class BrokerQueue:
    def __init__(self, maxsize):
        selfUEUE_MAX_SIZE = maxsize # 消息队列的最大容量
        self.__q = queue.Queue(maxsize = selfUEUE_MAX_SIZE) # 创建一个FIFO（先进先出）队列
...
class BrokerServer(BrokerQueue):
    """构建服务端，负责与客户端通信"""
    def __init__(self, service_ip='localhost', service_port=9999, maxsize=3):
        super(BrokerServer, self).__init__(maxsize)
